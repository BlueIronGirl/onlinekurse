...
//		 - Verify that the response "Location" header contains "http://localhost/accounts/21"
    ...
    public void accountSummary() throws Exception {
        List<Account> accounts = List.of(new Account("1234567890", "John Doe"));
    ...
    @Test
    public void testBeneficiaryDetails() throws Exception {
        Account account = new Account("1234567890", "John Doe");
        account.addBeneficiary("Test", new Percentage(0.1));

        given(accountManager.getAccount(anyLong())).willReturn(account);

        mockMvc.perform(get("/accounts/{accountId}/beneficiaries/{beneficiaryName}", 0L, "Test"))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
    ...
    @Test
    public void testBeneficiaryDetailsNotFound() throws Exception {
        Account account = new Account("1234567890", "John Doe");
        account.addBeneficiary("Test", new Percentage(0.1));

        given(accountManager.getAccount(anyLong())).willReturn(account);

        mockMvc.perform(get("/accounts/{accountId}/beneficiaries/{beneficiaryName}", 0L, "Test2"))
                .andExpect(status().isNotFound());
    ...
    public void createBeneficiary() throws Exception {
        mockMvc.perform(post("/accounts/{accountId}/beneficiaries", 0L)
                        .content("Test"))
                .andExpect(status().isCreated())
                .andExpect(header().string("Location", "http://localhost/accounts/0/beneficiaries/Test"));


        verify(accountManager).addBeneficiary(anyLong(), anyString());
    }
    ...
    public void removeBeneficiary() throws Exception {
        ...
        mockMvc.perform(delete("/accounts/{accountId}/beneficiaries/{beneficiaryName}", 0L, "Test"))
    ...
    @Test
    public void removeBeneficiaryNotExisting() throws Exception {
        Account account = new Account("1234567890", "John Doe");
        account.addBeneficiary("Test", new Percentage(0.1));

        given(accountManager.getAccount(anyLong())).willReturn(account);

        mockMvc.perform(delete("/accounts/{accountId}/beneficiaries/{beneficiaryName}", 0L, "Test2"))
                .andExpect(status().isNotFound());
